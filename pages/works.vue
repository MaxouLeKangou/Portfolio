...
<template>
    ...
    <div class="text-white grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5 lg:gap-8 px-4 sm:px-12">
        ...
        <NuxtLink :to="`/work/${work.uid}`" class="block w-full h-full">
          <div class="absolute bottom-4 left-6 flex flex-col gap-2 sm:gap-4">
            ...
            <div class="flex gap-2 text-xs sm:text-sm">
              <PrismicRichText :field="work.data.tag_1" class="py-[6px] px-3 sm:py-3 sm:px-5 bg-gray/30 rounded-full"/>
              <PrismicRichText :field="work.data.tag_2" class="py-[6px] px-3 sm:py-3 sm:px-5 bg-gray/30 rounded-full"/>
            </div>
          </div>
